...
class DictionaryImputer(BaseEstimator, TransformerMixin):
    """Dictionary Replace Imputer."""

    def __init__(self, variables=None):
        if not isinstance(variables, dict):
            raise InvalidModelInputError(
                f"Invalid dictionary format "
                'desired format is: "feature_name": {"target": value, "replace_value": value}'
            )
        else:
            self.variables = variables
...
class YearsVariableEstimator(BaseEstimator, TransformerMixin):
    """Years variable calculator."""
    ...
    def fit(self, X, y=None):
        self.imputer_today_ = pd.to_datetime('today')
